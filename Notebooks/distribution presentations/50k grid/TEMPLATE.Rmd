...
plot_fun <- function(yr){
  sub_plot <- function(season){
    ggplot() +
      geom_sf(data = coast) +
      geom_sf(data = con_hull[year == yr & season == season],
              aes(geometry = polygons)) +
      geom_sf(data = abun_grid[year == yr & season == season],
              aes(geometry = geometry, fill = med_abun), color = NA) +
      geom_sf(data = com[com$year == yr & com$season == season,], size = 5, shape = 8) +
      coord_sf(xlim = c(-4e5, 4.6e5), ylim = c(3.4e6, 4.6e6)) +
      scale_fill_viridis_c(option = 'plasma') +
      labs(subtitle = season, fill = 'Med. \nabund.') +
      theme_minimal() +
      theme(legend.position = c(0.9, 0.25))
  }
  
  sub_plot('SPRING') + sub_plot('FALL')
}
